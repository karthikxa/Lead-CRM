...
async function deployLuxuryTemplate() {
  ...
  console.log('2. Deploying email.service.patched.js and patch_invite_standalone.js...');
  const patchRes = await sb.process.executeCommand(`
    ...
    docker cp patch_invite_standalone.js zed-server-1:/tmp/patch_invite_standalone.js
    docker exec zed-server-1 node /tmp/patch_invite_standalone.js
...
deployLuxuryTemplate().catch(console.error);
