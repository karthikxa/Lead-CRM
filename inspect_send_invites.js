...
async function inspectSendInvitationsCode() {
  ...
  const res = await sb.process.executeCommand(`
    docker exec zed-server-1 node -e "
      ...
      const idx = c.indexOf('async sendWorkspaceInvitations');
      console.log(c.substring(idx, idx + 4000));
    "
  `);
  console.log('sendWorkspaceInvitations method:\n', res.result);
}

inspectSendInvitationsCode().catch(console.error);
